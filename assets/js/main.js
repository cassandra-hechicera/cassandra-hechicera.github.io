(function($) {
	...
	$(function() {
		...
		// Poptrox.
			...
			$( "#santa-muerte a:nth-child(" + (Math.floor(Math.random() * 4) + 1)  + ")" ).fadeIn();

			$( "#san-miguel a:nth-child(" + (Math.floor(Math.random() * 4) + 1)  + ")" ).fadeIn();

			$( "#virgen-guadalupe a:nth-child(" + (Math.floor(Math.random() * 2) + 1)  + ")" ).fadeIn();
			
			var messageSent = QueryString["message-sent"];
			
			if (messageSent == 'true') {
				toastr.success('Tu mensaje fue enviado');
			}
